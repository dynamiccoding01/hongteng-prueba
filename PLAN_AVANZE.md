...
sin código de requerimiento asociado.
...
## Checklist Sprint 8 (detalle de trabajo)

Pedido del dueño del proyecto: interfaz más simple, tema claro/oscuro seleccionable y una sección
de Ajustes con selector de idioma español/chino. Restricción explícita: sin tocar funcionalidades
ni base de datos — 100 % capa de presentación, cero migraciones nuevas.

**Alcance del idioma, confirmado con el usuario:** solo se traduce la interfaz fija (menú lateral
y botones comunes como Guardar/Cerrar/Editar). El contenido de cada página sigue en español por
ahora; se puede ampliar página por página más adelante.

- [x] ~~Tema claro/oscuro seleccionable: cookie `tema` (claro/oscuro/sistema, sin tocar la base de datos), `@custom-variant dark` en `app/globals.css` ligado a `data-theme`, estampado server-side en `app/layout.tsx` sin parpadeo (explícito) + script inline de 5 líneas para resolver «sistema» antes del primer pintado~~
- [x] ~~Selector de idioma es/zh: cookie `idioma`, diccionario en `lib/i18n.ts` (menú + botones comunes), `Context` (`components/proveedor-idioma.tsx`) sembrado desde `app/(app)/layout.tsx` para que el componente cliente `FormularioDesplegable` no tenga que leer `document.cookie` (evita parpadeo de texto al hidratar)~~
- [x] ~~Página nueva «Ajustes» (`app/(app)/ajustes`): selector de tema y de idioma, sin permiso requerido (preferencia personal, no acceso a datos)~~
- [x] ~~`components/navegacion.tsx`: menú traducido vía diccionario, resaltado de la ruta activa (`components/enlace-nav.tsx`, no existía antes), selector de tema en el pie~~
...
| 2026-07-23 | Seguridad | Rol **Superadmin** (desarrollo/TI): migración `0013` con banderas `acceso_total`/`protegido`, `es_superadmin()`, `es_rol_protegido()` y RLS de jerarquía (un Administrador no ve ni puede asignarse el rol protegido, ni por id literal — hueco de escalada detectado y cerrado con `security definer`). Matriz `/admin/roles` ahora **editable** (marcar/desmarcar permisos) y alta de roles. 7 tests de integración de RLS. Rama `feat/rol-superadmin` |
